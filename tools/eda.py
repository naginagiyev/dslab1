...
class EDA:
    ...
    def analyzeNumeric(self, col: str) -> str:
        ...
        if self.colTypes.get(col) == "numeric_discrete" and uniqueCount <= 20:
            valueCounts = (
    ...
    def analyzeDatetime(self, col: str) -> str:
        ...
        months = nonNull.dt.month_name().value_counts().head(3)
        ...
        lines = [
            ...
            f"- Max gap: {maxGap}",
            f"- Mean gap: {meanGap}",
            f"- Top years: {', '.join(f'{k} ({v})' for k, v in years.items())}",
            f"- Top months: {', '.join(f'{k} ({v})' for k, v in months.items())}",
            f"- Top weekdays: {', '.join(f'{k} ({v})' for k, v in weekdays.items())}",
        ]
        return "\n".join(lines)
    ...
    def sectionMissing(self) -> str:
        ...
        section.append(
            f"- Columns with 0% missing: {len(no_missingCols)} of {len(df.columns)}"
        )
            ...
            if targetType in {"regression", "time-series"}:
                ...
                if targetUnique == 2:
                    ...
                    for col in numericCols:
                        ...
                        if pd.notna(stat) and abs(stat) > 0.1:
