...
class EDA:
    ...
    def __init__(self, inputPath: str, targetCol: str, taskType: str):
        ...
        allowedTaskTypes = {"binary-classification", "regression", "multi-class-classification",
    ...
    def inferTypes(self) -> None:
        ...
        for col in df.columns:
            ...
            # if almost all values are unique, it is an id-like column
            if uniqueCount == nRows or uniqueRatio > 0.99:
                self.colTypes[col] = "id_like"
                continue

            # if the column is numeric-like in string format, convert it to numeric
            ...
            # checks if the unique values are only 2, if yes, it is a boolean column
            if self.isBooleanSeries(nonNull):
                self.colTypes[col] = "boolean"
                continue

            # checks if the column is a datetime column, it is a datetime column
            if pd.api.types.is_datetime64_any_dtype(series):
                self.colTypes[col] = "datetime"
                continue

            # if the column is a string or object type
            if pd.api.types.is_object_dtype(series) or pd.api.types.is_string_dtype(series):
                # checks if the datetime parse success ratio is greater than 0.80
                # if yes it is a datetime column
                if self.datetimeParseSuccessRatio(nonNull) > 0.80:
                    self.colTypes[col] = "datetime"
                    continue
                # checks if the average word count is greater than 3 and the unique ratio is greater than 0.5
                # if yes it is a text column
                avgWords = self.averageWordCount(nonNull)
                if avgWords > 3 and uniqueRatio > 0.5:
                    self.colTypes[col] = "text"
                    continue

                # checks if the column has mixed types, if yes it is a categorical column
            ...
            # if the column is a numeric column
            if pd.api.types.is_numeric_dtype(series):
                # Use a percentage of total rows instead of a fixed unique-count threshold.
                if uniqueRatio <= 0.05:
    ...
    def sectionTarget(self) -> str:
        ...
        if taskType in {"binary-classification", "multi-class-classification", "clustering", "anomaly-detection"}:
            ...
            if len(counts) > 0:
                minorityRatio = counts.min() / total if total else 0.0
                if minorityRatio < 0.10:
                    self.addWarning(
                        f"Class imbalance in target '{self.targetCol}': minority class is {minorityRatio * 100:.2f}%."
                    )
    ...
    def analyzeId(self, col: str) -> str:
        ...
        sampleValues = series.dropna().astype(str).head(5).tolist()
        ...
        lines = [
            ...
            f"- Sample values (up to 5): {sampleValues if sampleValues else 'N/A'}",
        ]
        return "\n".join(lines)
    ...
    def sectionToc(self) -> str:
        generatedAt = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        reportTitle = f"# EDA Report: {Path(self.inputPath).name}"
        ...
        tocLines = [reportTitle, f"Generated: {generatedAt}", "", "## Table of Contents", ""]
        for level, text in headers:
            indent = "  " if level == 3 else ""
            anchor = self.markdownAnchor(text)
            tocLines.append(f"{indent}- [{text}](#{anchor})")
